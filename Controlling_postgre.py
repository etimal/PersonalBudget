...
def DataCollection(paths:list):
    ...
    for file_path in paths:
        ...
        accounts, num_accts = get_accounts(xlsx) #number of accounts change based on defined accounts in each file
...
def ReadSource(run_mode:int):
    ...
    data = {column_a : [expenses.name, incomes.name, investment.name, investment_ytd.name, accounts_balance.name],
            column_b : [expenses.shape, incomes.shape, investment.shape, investment_ytd.shape, accounts_balance.shape],
    ...
    if run_mode == 1:
        ...
        for df in [expenses, incomes, investment, investment_ytd, accounts_balance]:
